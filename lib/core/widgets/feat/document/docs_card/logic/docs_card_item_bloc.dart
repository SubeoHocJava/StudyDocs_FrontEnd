...
/// Bloc cho 1 card — like, bookmark, download. Optimistic update, lỗi thì revert.
/// Dùng UseCase (giống DocumentCommentBloc), không gọi Repository trực tiếp.
class DocsCardItemBloc extends Bloc<DocsCardItemEvent, DocsCardItemState> {
  ...
  DocsCardItemBloc({
    ...
    on<CardErrorCleared>((event, emit) => emit(state.copyWith(lastError: null)));
  }

  void _onLiked(CardLiked event, Emitter<DocsCardItemState> emit) async {
  ...
  void _onBookmarked(CardBookmarked event, Emitter<DocsCardItemState> emit) async {
    final previous = state.doc;
    emit(state.copyWith(doc: previous.copyWith(isBookmarked: true)));
  ...
  void _onDownloaded(CardDownloaded event, Emitter<DocsCardItemState> emit) async {
